...
# Added prec_start argument (inicial value of precision parameter)
karma<- function (yab, ar=NA, ma=NA, a=0, b=1,link="logit",diag=1,h=6,X=NA,X_hat=NA,resid=3,prec_start=10)
{  
  ...
  fit1 <- karma.fit(
    y, ar, ma, a, b, link1, names_phi, names_theta, 
    names_beta, diag, h, X, X_hat,resid=resid,prec_start=prec_start
    ) # model estimation
